...
func (tx *Tx) init() {
	tx.ContactParents = NewContactParentsClient(tx.config)
	tx.Form = NewFormClient(tx.config)
	tx.StockManager = NewStockManagerClient(tx.config)
}
...
// applies a query, for example: ContactParents.QueryXXX(), the query will be executed
